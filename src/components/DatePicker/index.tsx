...
const DatePicker = ({
  ...
  const [days] = React.useState<string[]>(dayNames?.length === 7 ? dayNames : DAY_NAMES)
  const [months] = React.useState<string[]>(monthNames?.length === 12 ? monthNames : MONTH_NAMES)
  ...
  return (
    <div className={styles.darkbox} ref={dbRef}>
      <div className={styles.lightbox} ref={lbRef}>
        ...
        >
          {showTitle && <h4 className={styles.title}>{title || 'Select Date'}</h4>}
          <span className={styles.monthName}>{getHeader()}</span>
          <br />
          <span className={styles.year}>{selectedDate ? selectedDate.getFullYear() : year}</span>
        </div>
          ...
          <div className={styles.calendar}>
            {calendar.map((day, index) => (
              ...
              >
                <button
                  style={{
                    backgroundColor: selectedDate?.getTime() === day.getTime() ? colorScheme : '#fff',
                    color: selectedDate?.getTime() === day.getTime() ? '#fff' : '#000',
                  }}
