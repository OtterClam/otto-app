...
const WethSwap = () => {
  ...
  return (
    <StyledWethSwap>
      <TokenInput eth token={globals.WETH} value={value} setValue={(v: string) => setValue(v)} />
      <Button click={() => wrap({ value: utils.parseEther(value) })} loading={wrapState.status === 'Mining'}>
        Wrap ETH
      </Button>
    </StyledWethSwap>
  )
